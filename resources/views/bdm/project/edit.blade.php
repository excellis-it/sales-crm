@if (isset($type))
    <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasEdit" aria-labelledby="offcanvasRightLabel">
        ...
        <div class="offcanvas-body">
            <form action="{{ route('bdm.projects.update', $project->id) }}" method="post" data-parsley-validate=""
                ...
                <div class="row">
                    ...
                    <div id="edit-other-value" class="col-md-12 mb-3">
                        ...
                        @if ($project->projectMilestones->count() > 0)
                            @foreach ($project->projectMilestones as $key => $milestone)
                                <div class="row">
                                    ...
                                    {{-- if condition under html class --}}
                                    <div class="fetch-payment-hide-{{ $key }}">
                                        @if($milestone->payment_status=='Paid')
                                        <div class="col-md-12 mb-3">
                                            <div style="display: flex">
                                                <input type="date" name="milestone_payment_date[]"
                                                    class="form-control {{ $milestone->payment_status == 'Paid' ? 'disable-input' : '' }} picker" value="{{ $milestone->payment_date }}"
        ...
        $(document).ready(function() {
            ...
            $('#project_type_other').on('change', function() {
                ...
                if (project_type.includes('Other')) {
                    ...
                    $('#edit-other-value').html(html);
                } else {
                    $('#edit-other-value').html('');
                }
            });
        });
    </script>
    ...
    <script>
        $('.edit-milestone-print').on('click', function() {
            ...
            for (let index = 1; index <= new_number_of_milestone; index++) {
                ...
                html += '<div style="display: flex">';
                html +=
                    '<select name="payment_status[]" id="payment_status" class="form-control payment-status" data-id="' + index +'" required data-parsley-trigger="keyup"><option value="" disabled >Select Payment Status</option><option value="Paid">Paid</option><option value="Due" selected>Due</option></select>';
                html += '</div>';
                html += '</div>';
                ...
                html += '<div class="col-md-12 mb-3">';
                html += '<div style="display: flex">';
                html += '<select name="milestone_payment_mode[]" class="form-control" id="edit-milestone-mode-'+index+'"  data-parsley-trigger="keyup">';
