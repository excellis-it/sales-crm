        ...
        @if(count($tender_projects) > 0)
            @foreach($tender_projects as $key => $tender)
                <tr>
                    ...
                    <td>{{ $tender->tenderUser->name ?? 'N/A' }}</td>
                    <td class="text-end">
                        <div class="dropdown dropdown-action">
                            ...
                            <div class="dropdown-menu dropdown-menu-right">
                                ...
                                <a class="dropdown-item" href="{{ route('admin.tender-projects.edit', $tender->id) }}"><i
                                        class="fa fa-pencil m-r-5"></i> Edit</a>
                                <form action="{{ route('admin.tender-projects.destroy', $tender->id) }}" method="POST" style="display:inline-block;">
                                    @csrf
                                    @method('DELETE')
