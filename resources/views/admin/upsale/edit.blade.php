<div class="p-4">
    <form action="{{ route('admin.upsale.update', $upsale->id) }}" method="POST" id="upsale-edit-form">
        ...
        <div class="col-md-12 mb-3 text-end">
            <button type="submit" class="btn submit-btn px-5" id="btn-update-upsale">Update Upsale</button>
        </div>
    </form>
</div>
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status === 'success') {
                    ...
                    bootstrap.Offcanvas.getInstance(document.getElementById('offcanvasUpsaleEdit')).hide();
                    location.reload(); 
                }
            },
            error: function(xhr) {
                $btn.prop('disabled', false).text('Update Upsale');
                if (xhr.status === 422) {
