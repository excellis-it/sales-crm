...
class TenderProjectController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->search) {
            $tender_projects->where(function($q) use ($request) {
    ...
    {
        $statuses = TenderStatus::where('status', 1)->get();
        $tender_users = User::role('TENDER_USER')->get();
        return view('admin.tender_project.create', compact('statuses', 'tender_users'));
    }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'tender_user_id' => 'required|exists:users,id',
        ...
        try {
            ...
            $tender = TenderProject::create($request->all());
    ...
    {
        $tender = TenderProject::with('milestones')->findOrFail($id);
        $statuses = TenderStatus::where('status', 1)->get();
        $tender_users = User::role('TENDER_USER')->get();
        return view('admin.tender_project.edit', compact('tender', 'statuses', 'tender_users'));
    }
            ...
            if ($request->milestones) {
                foreach ($request->milestones as $milestone) {
                    if (!empty($milestone['milestone_name'])) {
                        ...
                        // Save Milestone Comment to Followups if new
                        if (!empty($milestone['milestone_comment'])) {
                            // Only add if it's different from previous ones or just always add as per requirement?
                            // Requirement says: When a Milestone Comment is added, it will also be stored in the tender_followups table.
                            // During update, we might be adding new ones.
                            TenderFollowup::create([
                                'tender_project_id' => $tender->id,
                                'comment' => $milestone['milestone_comment'],
                                'user_id' => Auth::id(),
    ...
    public function getFollowups($id)
    {
        ...
        $view = View::make('admin.tender_project.followups_modal_content', compact('followups'))->render();
