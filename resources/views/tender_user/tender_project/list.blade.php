...
@section('content')
    ...
    <div class="page-wrapper">
        ...
        <div class="page-header">
            <div class="row align-items-center">
                ...
                <div class="col-auto float-end ms-auto">
                    <a href="{{ route('tender-user.tender-projects.create') }}" class="btn add-btn"><i class="fa fa-plus"></i> Add Project</a>
                </div>
            </div>
        </div>
        <!-- /Page Header -->
        ...
        <!-- Search Filter -->
        <form action="{{ route('tender-user.tender-projects.index') }}" method="GET" id="search_form">
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="table-responsive" id="tender_project_table">
                    @include('tender_user.tender_project.table')
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#btn_refresh').click(function() {
                ...
                fetchData("{{ route('tender-user.tender-projects.index') }}");
            });
            ...
            function loadFollowups(id) {
                $.ajax({
                    url: "{{ route('tender-user.tender-projects.followups', ':id') }}".replace(':id', id),
                    type: 'GET',
                    success: function(response) {
            ...
            $('#add_followup_form').submit(function(e) {
                ...
                $.ajax({
                    url: "{{ route('tender-user.tender-projects.add-followup') }}",
                    type: 'POST',
                    data: formData,
                    success: function(response) {
                        if (response.success) {
